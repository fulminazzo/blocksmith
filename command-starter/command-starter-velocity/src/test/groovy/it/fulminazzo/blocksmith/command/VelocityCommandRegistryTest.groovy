...
class VelocityCommandRegistryTest extends Specification {
    ...
    def 'test that onUnregister of not existing command does not throw'() {
        when:
        registry.onUnregister('help')
    ...
    def 'test that wrapSender instantiates a new Velocity CommandSenderWrapper'() {
        given:
    ...
    def 'test that sender type is correct'() {
        expect:
        registry.senderType == CommandSource
    }
