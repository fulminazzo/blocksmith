...
import static it.fulminazzo.blocksmith.checker.FunctionalTestUtils.message
import static it.fulminazzo.blocksmith.checker.FunctionalTestUtils.runCheck
...
class TypesOrderCheckFunctionalTest extends Specification {
    ...
    def 'test that type #modifier after #other throws'() {
        ...
        violation.line == 8
        violation.column == 5
        violation.message == message(TypeValidator.valueOf(modifier.toUpperCase().trim()).errorMessage)
    ...
    def 'test that types check works on nested classes'() {
        ...
        nestViolation.message == message(VisibilityValidator.PUBLIC.errorMessage)
